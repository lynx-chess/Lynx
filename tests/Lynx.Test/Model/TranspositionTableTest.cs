...
public class TranspositionTableTests
{
    ...
    public void RecordHash_ProbeHash(int recordedEval, NodeType recordNodeType, int expectedProbeEval)
    {
        ...
        transpositionTable.RecordHash(position, halfMovesWithoutCaptureOrPawnMove: 5, staticEval, depth: 5, ply: 3, score: recordedEval, nodeType: recordNodeType, false, move: 1234);

        var ttEntry = transpositionTable.ProbeHash(position, ply: 3, halfMovesWithoutCaptureOrPawnMove: 5);
    ...
    public void RecordHash_ProbeHash_CheckmateSameDepth(int recordedEval)
    {
        ...
        transpositionTable.RecordHash(position, halfMovesWithoutCaptureOrPawnMove: 5, recordedEval, depth: 10, ply: sharedDepth, score: recordedEval, nodeType: NodeType.Exact, false, move: 1234);

        var ttEntry = transpositionTable.ProbeHash(position, ply: sharedDepth, halfMovesWithoutCaptureOrPawnMove: 5);
    ...
    public void RecordHash_ProbeHash_CheckmateDifferentDepth(int recordedEval, int recordedDeph, int probeDepth, int expectedProbeEval)
    {
        ...
        transpositionTable.RecordHash(position, halfMovesWithoutCaptureOrPawnMove: 5, recordedEval,  depth: 10, ply: recordedDeph, score: recordedEval, nodeType: NodeType.Exact, false, move: 1234);

        var ttEntry = transpositionTable.ProbeHash(position, ply: probeDepth, halfMovesWithoutCaptureOrPawnMove: 5);
        Assert.AreEqual(expectedProbeEval, ttEntry.Score);
        Assert.AreEqual(recordedEval, ttEntry.StaticEval);
    }
}