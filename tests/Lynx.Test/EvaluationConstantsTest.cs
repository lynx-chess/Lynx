...
public class EvaluationConstantsTest
{
    ...
    private readonly int _sensibleEvaluation =
        (2 * (Math.Max(MiddleGameBishopTable[0].Max(), EndGameBishopTable[0].Max()) + Utils.UnpackMG(BishopMobilityBonus[13]))) +
        (2 * (Math.Max(MiddleGameKnightTable[0].Max(), EndGameKnightTable[0].Max()))) +
        (2 * (Math.Max(MiddleGameRookTable[0].Max(), EndGameRookTable[0].Max()) + OpenFileRookBonus) + Utils.UnpackMG(SemiOpenFileRookBonus)) +
        (9 * (Math.Max(MiddleGameQueenTable[0].Max(), EndGameQueenTable[0].Max()) + Utils.UnpackMG(QueenMobilityBonus) * 64)) +
        (1 * (Math.Max(MiddleGameKingTable[0].Max(), EndGameKingTable[0].Max()) + Utils.UnpackMG(KingShieldBonus) * 8)) +
