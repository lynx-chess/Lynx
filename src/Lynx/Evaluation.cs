...
public partial class Position
{
    ...
    public (int Score, int Phase) StaticEvaluation(int movesWithoutCaptureOrPawnMove, PawnTableElement[] pawnEvalTable, ref EvaluationContext evaluationContext)
    {
        ...
        var totalPawnsCount = (whitePawns | blackPawns).CountBits();
        ...
        if (gamePhase <= 5)
        {
            ...
            if (totalPawnsCount == 0)
            {
                switch (gamePhase)
                {
                    ...
                    case 3:
                        {
                            var sideWithMorePieces = Utils.PieceOffset(_occupancyBitBoards[(int)Side.White].CountBits() >= _occupancyBitBoards[(int)Side.Black].CountBits());

                            if (_pieceBitBoards[(int)Piece.N + sideWithMorePieces].CountBits() == 2)      // NN vs N, NN vs B
                            {
                                return (0, gamePhase);
                            }
            ...
            else
            {
                ...
                if (gamePhase == 1)
                {
                    if (_pieceBitBoards[(int)Piece.B + sideWithMorePieces] != 0
                        && (_pieceBitBoards[(int)Piece.P + sideWithMorePieces] & Constants.NotAorH) == 0)
                    {
