...
public static class MoveExtensions
{
    ...
    public static bool TryParseFromUCIString(Position position, ReadOnlySpan<char> UCIString, ReadOnlySpan<Move> moveList, [NotNullWhen(true)] out Move? move)
    {
        ...
        for (int i = 0; i < moveList.Length; ++i)
        {
            ...
            var candidateTargetSquare = candidateMove.TargetSquare();

            var sameSourceSquare = candidateMove.SourceSquare() == sourceSquare;
            var sameTargetSquare = candidateTargetSquare == targetSquare;

            var matchMoveCondition = sameSourceSquare && sameTargetSquare;

            if (Configuration.EngineSettings.IsChess960)
            {
                var side = (int)(candidateMove.Piece() <= (int)Model.Piece.K ? Side.White : Side.Black);

                // FRC castling encoding: KxR
                matchMoveCondition |=
                    sameSourceSquare
                    && ((candidateMove.IsShortCastle()
                            && candidateTargetSquare == Utils.ShortCastleKingTargetSquare(side)
                            && targetSquare == position._initialKingsideRookSquares[side])
                        || (candidateMove.IsLongCastle()
                            && candidateTargetSquare == Utils.LongCastleKingTargetSquare(side)
                            && targetSquare == position._initialQueensideRookSquares[side]));
            }

            if (matchMoveCondition)
            {
