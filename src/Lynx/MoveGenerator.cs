...
#pragma warning restore IDE0350 // Use implicitly typed lambda
    ...
    /// </summary>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    internal static void GenerateCastlingMoves(ref int localIndex, Span<Move> movePool, Position position, ref readonly EvaluationContext evaluationContext)
    {
        ...
        if (castlingRights != default)
        {
            ...
            {
                var blackAttacks = evaluationContext.AttacksBySide[(int)Side.Black];

                bool canGenerateShortCastle, canGenerateLongCastle;

                if (blackAttacks != 0)
                {
                    canGenerateShortCastle =
                        (position.Castle & (int)CastlingRights.WK) != default
                        && (occupancy & Constants.WhiteShortCastleFreeSquares) == 0
                        && (blackAttacks & Constants.WhiteShortCastleNonAttackableSquares) == 0;

                    canGenerateLongCastle =
                        (position.Castle & (int)CastlingRights.WQ) != default
                        && (occupancy & Constants.WhiteLongCastleFreeSquares) == 0
                        && (blackAttacks & Constants.WhiteLongCastleNonAttackableSquares) == 0;
                }
                else
                {
            ...
            {
                var whiteAttacks = evaluationContext.AttacksBySide[(int)Side.White];
