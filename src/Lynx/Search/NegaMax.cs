            ...
            else
            {
                    ...
                    && scores[moveIndex] < EvaluationConstants.PromotionMoveScoreValue) // Quiet move
                {
                    ...
                    if (depth <= Configuration.EngineSettings.LMP_MaxDepth
                        && visitedMovesCounter >= Configuration.EngineSettings.LMP_BaseMovesToTry + (Configuration.EngineSettings.LMP_MovesDepthMultiplier * depth)) // Based on formula suggested by Antares
                    {
                        // After making a move
