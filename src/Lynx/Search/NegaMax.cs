...
public sealed partial class Engine
{
        ...
        bool parentWasNullMove = false, bool isVerifyingSE = false)
    {
        ...
        else if (!pvNode && !isInCheck && !isVerifyingSE)
        {
            ...
            {
                (rawStaticEval, phase, evaluationContext) = position.StaticEvaluation(Game.HalfMovesWithoutCaptureOrPawnMove, _pawnEvalTable);
...
#pragma warning restore S3949 // Calculations should not overflow
        ...
        {
            (rawStaticEval, _, evaluationContext) = position.StaticEvaluation(Game.HalfMovesWithoutCaptureOrPawnMove, _pawnEvalTable);
            staticEval = CorrectStaticEvaluation(position, rawStaticEval);
...
#pragma warning restore S2234 // Arguments should be passed in the same order as the method parameters
    ...
    public int QuiescenceSearch(int ply, int alpha, int beta, bool pvNode, CancellationToken cancellationToken)
    {
        ...
        */
        (var rawStaticEval, var _, var evaluationContext) = position.StaticEvaluation(Game.HalfMovesWithoutCaptureOrPawnMove, _pawnEvalTable);
