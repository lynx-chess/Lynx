...
public sealed partial class Engine
{
    ...
    internal int ScoreMove(Position position, Move move, int ply, ref EvaluationContext evaluationContext, ShortMove bestMoveTTCandidate = default)
    {
        ...
        if (isCapture)
        {
            ...
            return baseCaptureScore
                ...
                + CaptureHistoryEntry(move, evaluationContext.AttacksBySide[Utils.OppositeSide((int)position.Side)]);
        }
    ...
    /// </summary>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private void UpdateMoveOrderingHeuristicsOnCaptureBetaCutoff(int depth, ReadOnlySpan<int> visitedMoves, int visitedMovesCounter, int move, Bitboard oppositeSideAttacks)
    {
        var rawHistoryBonus = HistoryBonus[depth];
        var rawHistoryMalus = HistoryMalus[depth];

        ref var captureHistoryEntry = ref CaptureHistoryEntry(move, oppositeSideAttacks);
        ...
        for (int i = 0; i < visitedMovesCounter; ++i)
        {
            ...
            if (capturedPiece != (int)Piece.None)
            {
                ref var captureHistoryVisitedMove = ref CaptureHistoryEntry(visitedMove, oppositeSideAttacks);
