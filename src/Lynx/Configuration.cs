...
public sealed class EngineSettings
{
    ...
    [SPSA<int>(3, 10, 0.5)]
    public int LMR_MinDepth { get; set; } = 5;

    [SPSA<int>(1, 10, 0.5)]
    public int LMR_MinFullDepthSearchedMoves { get; set; } = 9;

    /// <summary>
    /// Value originally from Stormphrax, who apparently took it from Viridithas
    /// </summary>
    [SPSA<double>(0.1, 2, 0.10)]
    public double LMR_Base { get; set; } = 1.39;

    /// <summary>
    /// Value originally from Akimbo
    /// </summary>
    [SPSA<double>(1, 5, 0.1)]
    public double LMR_Divisor { get; set; } = 3.77;
    ...
    [SPSA<int>(1, 10, 0.5)]
    public int NMP_DepthDivisor { get; set; } = 2;

    [SPSA<int>(5, 30, 1)]
    public int AspirationWindow_Base { get; set; } = 6;

    //[SPSA<int>(5, 30, 1)]
    //public int AspirationWindow_Delta { get; set; } = 13;

    [SPSA<int>(1, 20, 1)]
    public int AspirationWindow_MinDepth { get; set; } = 12;

    [SPSA<int>(1, 10, 0.5)]
    public int RFP_MaxDepth { get; set; } = 4;

    [SPSA<int>(1, 300, 15)]
    public int RFP_DepthScalingFactor { get; set; } = 203;
    ...
    [SPSA<int>(1, 300, 15)]
    public int Razoring_NotDepth1Bonus { get; set; } = 166;

    [SPSA<int>(1, 10, 0.5)]
    public int IIR_MinDepth { get; set; } = 9;

    [SPSA<int>(1, 10, 0.5)]
    public int LMP_MaxDepth { get; set; } = 9;
    ...
    [SPSA<int>(1, 200, 10)]
    public int FP_DepthScalingFactor { get; set; } = 2;

    [SPSA<int>(0, 500, 25)]
    public int FP_Margin { get; set; } = 248;

    [SPSA<int>(0, 10, 0.5)]
    public int HistoryPrunning_MaxDepth { get; set; } = 2;

    [SPSA<int>(-8192, 0, 512)]
    public int HistoryPrunning_Margin { get; set; } = -1607;
}
