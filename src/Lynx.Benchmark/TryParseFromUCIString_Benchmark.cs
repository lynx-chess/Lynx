...
public class TryParseFromUCIString_Benchmark : BaseBenchmark
{
    ...
    [Benchmark]
    [ArgumentsSource(nameof(Data))]
    public Game Memory(string positionCommand)
    {
        return PositionCommand.ParseGame(positionCommand, MemoryPool<Move>.Shared.Rent(Constants.MaxNumberOfPossibleMovesInAPosition).Memory.Span);
    }
}